...
> Ordinal has ~20 event types and no test-event trigger, so rather than
> hand-write payloads, [`scripts/ordinal/docs.ts`](../../scripts/ordinal/docs.ts)
> scrapes Ordinal's documentation: it discovers every event page from the
> docs index (`llms.txt`) and extracts the canonical example payload each
> page publishes in a `json` fence, then writes it here in this repo's
...
A subset can additionally be refreshed with **real** deliveries by running
`yarn capture:ordinal` against a live Ordinal workspace (see
[`scripts/ordinal/`](../../scripts/ordinal/)) — this performs the matching
API operation and records the actual webhook. The rest require a side
channel the API can't drive unattended and stay docs-sourced. The table
below tracks which is which.
...
## Regenerating / re-capturing

```sh
yarn generate:ordinal  # rebuild ALL payloads from Ordinal's published docs (no creds)
# — or, for real deliveries from a live workspace —
yarn setup:ordinal     # one-time: Hookdeck source + Ordinal webhook subscription
yarn capture:ordinal   # overwrite the live-capturable subset with real captures
```
